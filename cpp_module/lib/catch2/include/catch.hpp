            ...
            m_chunk_size(size), m_generator(std::move(generator))
        {
            ...
            if (m_chunk_size != 0) {
                ...
                for (size_t i = 1; i < m_chunk_size; ++i) {
                    if (!m_generator.next()) {
                        Catch::throw_exception(GeneratorException("Not enough values to _initializeScheduler the first chunk"));
                    }
                    m_chunk.push_back(m_generator.get());
                }
            }
...
#    pragma GCC diagnostic ignored "-Wmissing-field-initializers"
    ...
    FatalConditionHandler::FatalConditionHandler() {
        assert(!altStackMem && "Cannot initializeScheduler POSIX signal handler when one already exists");
        if (altStackSize == 0) {
