...
def _getMaxStaggeringApplicableNextEpoch(departure, globalInstance, nextEpochInstance, originalVehicleID) -> float:
    firstOriginalArc = globalInstance.arcBasedShortestPaths[originalVehicleID][0]
...
def _addDeparturesToNextEpoch(nextEpochDepartures: list[NextEpochDeparture],
    ...
    for departure in nextEpochDepartures:
        ...
        lenPathNextEpoch = len(currentEpochInstance.arcBasedShortestPaths[departure.vehicle][departure.position:])
        pathToAppend = globalInstance.arcBasedShortestPaths[originalVehicleID][-lenPathNextEpoch:]
        lenTotalPath = len(currentEpochInstance.arcBasedShortestPaths[departure.vehicle])
        ...
        nextEpochInstance.arcBasedShortestPaths.append(pathToAppend)
...
def _getNextEpochDeparturesActiveVehicles(currentEpochStatusQuo: EpochSolution,
    ...
    for vehicleEpochID, schedule in enumerate(currentEpochStatusQuo.congestedSchedule):
        ...
        if timeInNextEpoch is not None:
            ...
            arcInNextEpoch = currentEpochInstance.arcBasedShortestPaths[vehicleEpochID][positionInNextEpoch]
