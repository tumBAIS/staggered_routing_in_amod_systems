...
def get_departure_in_next_epoch(current_epoch_instance: EpochInstance, schedule: list[float],
                                solver_params: SolverParameters) -> tuple[int, float] | \
                                                                    tuple[None, None]:
    is_in_next_epoch = lambda x: x / 60 > solver_params.epoch_size * (
...
def get_max_staggering_applicable_next_epoch(departure, global_instance, next_epoch_instance,
                                             original_vehicle_id, solver_params: SolverParameters) -> float:
...
def update_next_epoch_departures(current_epoch_instance: EpochInstance, current_epoch_status_quo: EpochSolution,
                                 vehicle_status_list: list[VehicleStatus],
                                 active_next_epoch_departures: list[NextEpochDeparture],
                                 solver_params: SolverParameters) -> list[NextEpochDeparture]:
...
def update_next_epoch_instance(current_epoch_instance: EpochInstance, current_epoch_status_quo: EpochSolution,
                               ...
                               solver_params: SolverParameters) -> EpochInstance:
