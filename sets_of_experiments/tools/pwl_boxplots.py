...
def get_pwl_boxplots(results_df: pd.DataFrame, path_to_figures: Path) -> None:
    ...
    def plot_pwl_boxplot(data, x_col, y_col, xlabel, folder_name, file_name, x_limits=None):
        ...
        if x_col == "absolute_delay_reduction_minutes":
            y_ticks = [f" " for _ in reversed(range(len(data[y_col].unique())))]
            plt.yticks(ticks=range(len(data[y_col].unique())), labels=y_ticks)
        else:
            y_ticks = [f"\\texttt{{PWL {i + 1}}}" for i in reversed(range(len(data[y_col].unique())))]
            plt.yticks(ticks=range(len(data[y_col].unique())), labels=y_ticks)
    ...
    plot_configs = [
        ...
        ("absolute_delay_reduction_minutes", "\$\\DelayReduction\$[min]", "absolute_reduction", None),
        ("relative_delay_reduction", "\$\\DelayReduction\$[%]", "relative_reduction", (0, 100))
    ]
